...
class TutorialPage extends HTMLElement {
  ...
  addEventListeners() {
    ...
    this.shadowRoot.addEventListener('info-modal-opened', () => {
  ...
  showStep(textKeyOverride = null) {
    const step = this.steps[this.currentStep];
    this.shadowRoot.getElementById('instruction-text').textContent =
      translationService.t(textKeyOverride || step.textKey, { title: configService.title });
